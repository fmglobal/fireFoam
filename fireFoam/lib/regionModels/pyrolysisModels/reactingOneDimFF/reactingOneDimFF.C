...
void reactingOneDimFF::readReactingOneDimControls()
{
    ...
    pyrHSource_ = coeffs().getOrDefault("pyrHSource", false);
...
void reactingOneDimFF::updatePhiPyr()
{
    phiHsPyr_ == dimensionedScalar(phiHsPyr_.dimensions(), Zero);
    phiGas_ == dimensionedScalar(phiGas_.dimensions(), Zero);

    rhoSpPyr_ == dimensionedScalar(rhoSpPyr_.dimensions(), Zero);

    const speciesTable& pyrTable = solidChemistry_->gasTable();

    forAll(pyrTable, pyrI)
    {
        tmp<volScalarField> tHsiPyr =
            solidChemistry_->gasHs(solidThermo_->p(), solidThermo_->T(), pyrI);

        const volScalarField& HsiPyr = tHsiPyr();

        const volScalarField::Internal& RRiPyr = solidChemistry_->RRg(pyrI);
        ...
        forAll(intCoupledPatchIDs_, i)
        {
            ...
            forAll(phiGasp, facei)
            {
                ...
                {
                    const label celli = cells[k];
                    massInt += RRiPyr[celli]*cellVol[celli];
                    phiHsPyr_[celli] += massInt*HsiPyr[celli];
                    rhoSpPyr_[celli] += massInt/patch.magSf()[facei];
                }
...
void reactingOneDimFF::updateFields()
{
    ...
    //Note: Commented out (?) as the sensible pyr energy is included in energy eq.
    updatePhiPyr();
...
void reactingOneDimFF::solveSpeciesMass()
{
    ...
    for (label i=0; i<Ys_.size()-1; i++)
    {
        ...
        /* See note in energy equation update
...
void reactingOneDimFF::solveEnergy()
{
    ...
    if (pyrHSource_)
    {
        const surfaceScalarField phiGas(fvc::interpolate(phiHsPyr_));
        hEqn += fvc::div(phiGas);
    }
...
/*
    ...
    Solid -> Pyr, thus the ddt term is compensated exactly by chemistrySh and
...
void reactingOneDimFF::calculateMassTransfer()
{
    ...
    {
        const label patchi = intCoupledPatchIDs_[i];
        totalPyrMassFlux_ += gSum(phiGas_.boundaryField()[patchi]);
    }
...
:
    ...
    Tmin_(200),
    Tmax_(5000),
    initial_thickness_(coeffs().lookupOrDefault<scalar>("thickness",0)),
    phiGas_
    (
        IOobject
        (
    ...
    addedPyrMass_(dimensionedScalar(dimMass, Zero)),
    totalPyrMassFlux_(0.0),
    ...
    charring_(true)  // Will be set to false if 'char' not found in species list
{
    ...
    //- When the mesh is not moving, safe to set minimumDelta to zero
    if (!moveMesh_)
    {
        minimumDelta_ = 0.0;
    }
    //- For tracking thickness/burnout
    else
    {
        updateRegionThickness();
    }
...
const volScalarField::Boundary& reactingOneDimFF::burnoutBf() const
{
    return thickness_.boundaryField();;
}


void reactingOneDimFF::updateRegionThickness()
{
        volScalarField::Boundary& thicknessBf = thickness_.boundaryFieldRef();
        ...
        forAll(intCoupledPatchIDs_, i)
        {
            ...
            scalarField& thickPyrp = thicknessBf[patchi];

            const scalarField& cellVol = regionMesh().V();

            forAll(thickPyrp, facei)
            {
                const labelList& cells = boundaryFaceCells_[totalFaceId++];
                scalar cumulative_thickness = 0.0;
                forAllReverse(cells, k)
                {
                    const label celli = cells[k];
                    cumulative_thickness += cellVol[celli]/patch.magSf()[facei];
                    thickness_[celli] = cumulative_thickness; // thickness_ stores cumulative value, back to front

                    // Burnout is fraction of initial thickness lost
                    // Only value at front cell has meaning - this is accessed by surfaceFieldValue operations
                    burnout_[celli] = 1 - cumulative_thickness/initial_thickness_; 
                }
        ...
        thickness_.correctBoundaryConditions();
        burnout_.correctBoundaryConditions();
}
...
void reactingOneDimFF::info()
{
    ...
    Info<< indent << "Total pyr mass produced  [kg] = "
        << addedPyrMass_.value() << nl
        << indent << "Total solid mass lost    [kg] = "
        << lostSolidMass_.value() << nl
        << indent << "Total pyrolysis pyres  [kg/s] = "
        << totalPyrMassFlux_ << nl
