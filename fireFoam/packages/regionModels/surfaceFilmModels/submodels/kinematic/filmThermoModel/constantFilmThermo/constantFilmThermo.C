...
:
    ...
    // Alex
    hs0_("hs0"),
    Tref_(coeffDict_.lookupOrDefault("Tref",295.15)),
    pRef_(coeffDict_.lookupOrDefault("pRef",101325.0))
    // Alex
{
    init(rho0_);
    init(mu0_);
...
// Alex
scalar constantFilmThermo::Tref() const 
{
    return Tref_;
}

scalar constantFilmThermo::pRef() const
{
    return pRef_;
}
// Alex
...
const
{
    if (!hs0_.set_)
    {
        coeffDict_.readEntry(hs0_.name_, hs0_.value_);
        hs0_.set_ = true;
    }
...
// Alex
tmp<volScalarField> constantFilmThermo::hs
(
...
const
{
    ...
    ths.ref().primitiveFieldRef() = this->hs(0, 0);
